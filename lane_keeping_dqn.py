...
class environment:
    def __init__(self, trace_paths, trace_config, car_config, sensor_config):
    ...
    def empty_action_space(self):
        ...
        curvature_range = np.arange(
            -0.2, 0.2 + curvature_increment, curvature_increment
        )
    ...
    def reward_3(self):
        ...
        W_d = self.agent.trace.road_width / 2.0
    ...
    def step(self, action, dt=1 / 30):
        ...
        road_half_width = self.agent.trace.road_width / 2.0
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--operation", type=str, nargs="+", help="Load or New", required=True
    )
    parser.add_argument(
        "--save-path",
        type=str,
    ...
    if args.operation[0].lower() == "new":
        ...
        while step < max_num_steps and not done:
            ...
            state_tensor = torch.from_numpy(state).unsqueeze(0).to(device)
            q_values = (
                target_network(state_tensor.permute(0, 3, 1, 2)).cpu().data.numpy()
            )
